...
//		---author:jg_xugj----start-----date:20151211--------for：#779 【菜单问题】当删了t_s_operation表中记录时， t_s_role_function 表中operation 字段应该同步更新。
	...
	private void updateSubFunction(List<TSFunction> subFunction, TSFunction parent) {
		if (subFunction.size() == 0) {// 没有子菜单是结束
			...
			for (TSFunction tsFunction : subFunction) {
				...
				List<TSFunction> subFunction1 = systemService.findByProperty(TSFunction.class, "TSFunction.id",
						tsFunction.getId());
	...
	public AjaxJson saveFunction(TSFunction function, HttpServletRequest request) {
		...
		if (StringUtil.isEmpty(function.getFunctionOrder())) {
			function.setFunctionOrder("0");
		}
		// 2020-04-30 空值会导致Hibernate尝试加载
		if("".equals(function.getTSIcon().getId())) {
			function.setTSIcon(null);
		}
		if("".equals(function.getTSIconDesk().getId())) {
			function.setTSIconDesk(null);
		}
	...
	public ModelAndView addorupdate(TSFunction function, HttpServletRequest req) {
		...
		if (function.getTSFunction() != null && function.getTSFunction().getId() != null) {
			...
			function.setTSFunction(
					(TSFunction) systemService.getEntity(TSFunction.class, function.getTSFunction().getId()));
		}
...
//        update-end-Author:zhangguoming  Date:20140914 for：菜单管理页面：菜单排序
	...
	public ModelAndView searchApp(TSFunction function, HttpServletRequest req) {
		...
		String menuListMap = "";
		// String menuListMap =
		// "<div class='appListContainer ui-sortable' customacceptdrop='0' index='0'
		// _olddisplay='block' style='width: auto; height: auto; margin-left: 10px;
		...
		if (functionList.size() > 0 && functionList != null) {
			for (int i = 0; i < functionList.size(); i++) {
				...
				menuListMap = menuListMap + "<div type='" + i + 1 + "' class='menuSearch_Info' id='"
						+ functionList.get(i).getId() + "' title='" + functionList.get(i).getFunctionName() + "' url='"
						+ functionList.get(i).getFunctionUrl() + "' icon='" + icon
	 ...
	 * 
	 * : addorupdaterule  TODO @param
	 * operation @param req @param 设定文件 @return ModelAndView
	 * 返回类型 @throws
	 */
	 ...
	 * 
	 * : opdategrid  TODO @param request @param
	 * response @param dataGrid 设定文件 @return void 返回类型 @throws
	 */
	 ...
	 * delrule 删除数据权限规则
	 * 
	 * : delrule  TODO @param operation @param
	 * request @param 设定文件 @return AjaxJson 返回类型 @throws
	 */
	 ...
	 * saverule保存规则权限值
	 * 
	 * : saverule  TODO @param operation @param
	 * request @param 设定文件 @return AjaxJson 返回类型 @throws
	 */
	...
	public int justHaveDataRule(TSDataRule dataRule) {
		String sql = "SELECT id FROM t_s_data_rule WHERE functionId='" + dataRule.getTSFunction().getId()
