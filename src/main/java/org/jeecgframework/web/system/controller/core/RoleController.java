...
//        update-start--Author:zhangguoming  Date:20140828 for：bug修复：角色列表，查看用户列表报错
		 ...
		 * subCq.setProjection(Property.forName("TSUser.id")); subCq.eq("TSRole.id",
		 * roleId); subCq.add();
		 * cq.add(Property.forName("id").in(subCq.getDetachedCriteria())); cq.add();
		 */
//        update-end--Author:zhangguoming  Date:20140828 for：bug修复：角色列表，查看用户列表报错
		Criterion cc = null;
		if (roleUser.size() > 0) {
	...
	public List<ComboTree> getRoleTree(HttpServletRequest request) {
		...
		List<TSRole[]> orgRoleArrList = systemService.findHql(
...
//      systemService.addLog(message, Globals.Log_Type_UPDATE, Globals.Log_Leavel_INFO);
	...
	public String exportXls(TSRole tsRole, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid,
			ModelMap modelMap) {
		...
		modelMap.put(NormalExcelConstants.PARAMS,
				new ExportParams("角色表列表", "导出人:" + ResourceUtil.getSessionUserName().getRealName(), "导出信息"));
	...
	public String exportXlsByT(TSRole tsRole, HttpServletRequest request, HttpServletResponse response,
		...
		modelMap.put(NormalExcelConstants.CLASS, TSRole.class);
		modelMap.put(NormalExcelConstants.PARAMS,
				new ExportParams("用户表列表", "导出人:" + ResourceUtil.getSessionUserName().getRealName(), "导出信息"));
	...
	@RequestMapping(params="roleUsers")
	public String roleUsers() {
		return "system/role/userPanel";
	}
}
