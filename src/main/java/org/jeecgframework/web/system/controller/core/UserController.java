...
import org.jeecgframework.poi.excel.NormalExcelConstants;
...
public class UserController extends BaseController {
	...
	@PostConstruct
	public void init() {
		String k1="account.defaultPassword";
		Config.ChangeHandler h1=(key)->{
			if(key==null || k1.equals(key))
				defaultPassword=Config.get(k1);
		};
		
		String k2="account.passwordValidateRegex";
		Config.ChangeHandler h2=(key)->{
			if(key==null || k2.equals(key))
				patternPassword=Pattern.compile(StringUtil.ifEmpty(Config.get(k2),"(?!^\\d+$)(?!^[a-zA-Z]+$)(?!^[_#@]+$).{8,}"));
		};
		Config.addChangeListener(h1);
		Config.addChangeListener(h2);
		h1.onChange(null);
		h2.onChange(null);
	}
	...
	@ResponseBody
	public JsonResult savenewpwd(HttpServletRequest request) {
		...
		if(!patternPassword.matcher(newpassword).matches()) {
			return new JsonResult("新密码复杂度过低");
		}
	...
	@ResponseBody
	public JsonResult savenewpwdforuser(HttpServletRequest req) {
...
//                update-end--Author:zhangguoming  Date:20140825 for：添加业务逻辑
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			TSUser euser = systemService.getEntity(TSUser.class, user.getId());
//			euser.setEmail(user.getEmail());
//			euser.setOfficePhone(user.getOfficePhone());
//			euser.setMobilePhone(user.getMobilePhone());
...
//            users.setTSDepart(user.getTSDepart());
			...
			euser.setStatus(Globals.User_Normal);
			euser.setOrderIndex(user.getOrderIndex());
			systemService.updateEntitie(euser);
			List<TSRoleUser> ru = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
			systemService.deleteAllEntitie(ru);
			message = "用户: " + euser.getUserName() + "更新成功";
			if (StringUtil.isNotEmpty(roleid)) {
				saveRoleUser(euser, roleid);
			}
			systemService.addLog(message, Globals.Log_Type_UPDATE, Globals.Log_Leavel_INFO);
		} else {
			...
			if (users != null) {
				...
				String password = defaultPassword;//默认123456//oConvertUtils.getString(req.getParameter("password"));
...
//    update-end--Author:zhangguoming  Date:20140825 for：添加新的业务逻辑方法
	...
	public ModelAndView goUpdate(String userid) {
		ModelAndView mv = new ModelAndView("system/user/user");
		TSUser guser = systemService.get(TSUser.class, userid);
		Map<String , Object> mapParam = new HashMap<String, Object>();
		mapParam.put("user_id", userid);
		//获取对应的角色 
		Map<String, Object> roleMap = queryService.queryObject(null, "user_role_map", mapParam);//systemService.findOneForJdbc("select wm_concat(to_char(role.id)) as roleid from  t_s_role_user roleus  inner join t_s_role role on roleus.roleid = role.id  where roleus.userid =?", userid);
		//获取对应的部门
		Map<String, Object> departMap = queryService.queryObject(null, "user_org_map", mapParam);//systemService.findOneForJdbc("select wm_concat(to_char(de.id)) as departid from t_s_depart de inner join t_s_user_org org on de.id = org.org_id where org.USER_ID=?", userid);
		mv.addObject("guser", guser);
		mv.addObject("groleid", roleMap.get("roleid"));
		mv.addObject("gdepartid", departMap.get("departid"));
		return mv;
	}
...
//				continue;
//			}
//			if(cookie.getName().equalsIgnoreCase("SUNZINDEXSTYLE")){
	...
	* @throws
	...
	public AjaxJson saveStyle(HttpServletRequest request,HttpServletResponse response) {
		...
		if(user!=null){
			...
			if(!StringUtil.isEmpty(indexStyle)){
				Cookie cookie = new Cookie("SUNZINDEXSTYLE", indexStyle);
				//设置cookie有效期为一个月
