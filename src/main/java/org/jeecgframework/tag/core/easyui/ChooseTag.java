...
public class ChooseTag extends TagSupport {
	...
	public StringBuffer end() {
		...
		sb.append("<a href=\"#\" class=\"easyui-linkbutton\" plain=\"true\" icon=\"" + icon + "\" onClick=\"choose_"+methodname+ StringUtil.replaceAll("()\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.select", langArg)));
		if (isclear&&StringUtil.isNotEmpty(textname)) {
			sb.append("<a href=\"#\" class=\"easyui-linkbutton\" plain=\"true\" icon=\"icon-redo\" onClick=\"clearAll_"+methodname+ StringUtil.replaceAll("();\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.clear", langArg)));
		}
		...
		sb.append("if(typeof(windowapi) == 'undefined'){");
			...
			sb.append("button : [ {");
			sb.append(StringUtil.replaceAll("name : \'{0}\',", "{0}", confirm));
			sb.append("callback : clickcallback_"+methodname+",");
			sb.append("focus : true");
			sb.append("}, {");
			sb.append(StringUtil.replaceAll("name : \'{0}\',","{0}", cancel));
