	...
	public void setColumn(String title, String field, Integer width, String rowspan, 
		...
		if (null!=operationCodes) {
			for (String MyoperationCode : operationCodes) {
				if (StringUtil.isEmpty(MyoperationCode))
					break;
		...
		if (!StringUtil.isEmpty(dictionary)&&(!popup)) {
...
//                update-end-Author:zhangguoming  Date:20140921 for：TASK #458 列表hidden=false，才是隐藏好像有点问题
	...
	public StringBuffer end() {
		...
		if(autoLoadData)
		   sb.append("url:\'" + actionUrl + "&field=" + fields + "\',");
			...
			sb.append("url:\'',");
		if(!StringUtil.isEmpty(rowStyler)){
			sb.append("rowStyler: function(index,row){ return "+rowStyler+"(index,row);},");
		}
		if(!StringUtil.isEmpty(extendParams)){
		...
		if(toolBarList.size()==0 && !hasQueryColum(columnList)){
			...
			sb.append("<div style=\"height:48px;\" class=\"datagrid-toolbar\">");
		}
		sb.append("<span style=\"float:left;\" >");
		if(toolBarList.size()>0)
		{
			for (DataGridUrl toolBar : toolBarList) {
				sb.append("<a href=\"javascript:;\" class=\"easyui-linkbutton\" plain=\"true\" icon=\""+toolBar.getIcon()+"\" ");
		...
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			...
			sb.append("<a href=\"javascript:;\" class=\"easyui-linkbutton\" iconCls=\"icon-search\" onclick=\""+  name+ StringUtil.replaceAll("search()\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.query")));
			sb.append("<a href=\"javascript:;\" class=\"easyui-linkbutton\" iconCls=\"icon-reload\" onclick=\"searchReset('"+name+ StringUtil.replaceAll("')\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.reset")) );
			if(queryBuilder){
				sb.append("<a href=\"javascript:;\" class=\"easyui-linkbutton\" iconCls=\"icon-search\" onclick=\"queryBuilder('"+ StringUtil.replaceAll("')\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.querybuilder")) );
			}sb.append("</span>");
	...
	private String extendAttribute(String field) {
		...
		try{
			...
			while(it.hasNext()){
				...
				try {
					JSONObject nextObj =(JSONObject) en.getValue();
					Iterator<String> itvalue =nextObj.keySet().iterator();
					re.append(key+"="+"\"");
					if(nextObj.size()<=1){
						String onlykey = itvalue.next();
	...
	protected void getOptUrl(StringBuffer sb) {
		...
		for (DataGridUrl dataGridUrl : list) {
			...
			if (OptTypeDirection.Confirm.equals(dataGridUrl.getType())) {
				sb.append("href+=\"[<a href=\'javascript:;\' onclick=confirm(\'" + url + "\',\'" + dataGridUrl.getMessage() + "\',\'"+name+"\')" + style.toString() + "> \";");
			}
			if (OptTypeDirection.Del.equals(dataGridUrl.getType())) {
				sb.append("href+=\"[<a href=\'javascript:;\' onclick=delObj(\'" + url + "\',\'"+name+"\')" + style.toString() + ">\";");
			}
			if (OptTypeDirection.Fun.equals(dataGridUrl.getType())) {
				...
				sb.append("href+=\"[<a href=\'javascript:;\' onclick=" + name + "(" + parmars + ")" + style.toString() + ">\";");
			}
			if (OptTypeDirection.OpenWin.equals(dataGridUrl.getType())) {
				sb.append("href+=\"[<a href=\'javascript:;\' onclick=openwindow('" + dataGridUrl.getTitle() + "','" + url + "','"+name+"'," + dataGridUrl.getWidth() + "," + dataGridUrl.getHeight() + ")" + style.toString() + ">\";");
			...
			if (OptTypeDirection.OpenTab.equals(dataGridUrl.getType())) {
				sb.append("href+=\"[<a href=\'javascript:;\' onclick=addOneTab('" + dataGridUrl.getTitle() + "','" + url  + "')>\";");
			}
	...
	protected void getField(StringBuffer sb,int frozen) {
		...
		for (DataGridColumn column : columnList) {
			...
			if(!StringUtil.isEmpty(column.getExtendParams())){
...
//            update-end-Author:zhangguoming  Date:20140921 for：TASK #458 列表hidden=false，才是隐藏好像有点问题
			...
			if (columnStyleList.size() > 0 && !column.getField().equals("opt")) {
				...
				for (ColumnValue columnValue : columnStyleList) {
					if (columnValue.getName().equals(column.getField())) {
						...
						if((value.length == 0||StringUtil.isEmpty(value[0]))&&text.length==1){
							if(text[0].indexOf("(")>-1){
	...
	private void installOperationCode(DataGridUrl dataGridUrl,String operationCode,List optList){
		...
		}else if(!StringUtil.isEmpty(operationCode)){
			Set<String> operationCodes = (Set<String>) super.pageContext.getRequest().getAttribute(Globals.OPERATIONCODES);
			if (null!=operationCodes) {
	...
	private String getAutoSpan(String filed,String extend){
        ...
        .append("</script>")
        .append("<input class=\"inuptxt\" type=\"text\" id=\""+getEntityName()+"_"+id+"\" name=\""+filed+"\"  "+extend+ StringUtil.replaceAll(" nullmsg=\"\" errormsg=\"{0}\"/>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("input.error")));
		return nsb.toString();
	}
	/**
	 * 获取实体类名称,没有这根据规则设置
...
//		sb.append("<link rel=\"stylesheet\" href=\"plug-in/easyui/themes/metro/main.css\" />");
		...
		if(toolBarList.size()>0)
		{
			for (DataGridUrl toolBar : toolBarList) {
				...
				sb.append("<a href=\"javascript:;\" class=\"button\" plain=\"true\" icon=\""+toolBar.getIcon()+"\" ");
				if(StringUtil.isNotEmpty(toolBar.getOnclick()))
				{
					sb.append("onclick="+toolBar.getOnclick()+"");
				}
		...
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			sb.append("<span style=\"float:right\">");
			sb.append("<a href=\"javascript:;\" class=\"button\" iconCls=\"icon-search\" onclick=\""+  name+ StringUtil.replaceAll("search()\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.query")));
			sb.append("<a href=\"javascript:;\" class=\"button\" iconCls=\"icon-reload\" onclick=\"searchReset('"+name+ StringUtil.replaceAll("')\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.reset")) );
			if(queryBuilder){
				sb.append("<a href=\"javascript:;\" class=\"button\" iconCls=\"icon-search\" onclick=\"queryBuilder('"+ StringUtil.replaceAll("')\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.querybuilder")) );
			}sb.append("</span>");
		}else if("single".equals(getQueryMode())&& hasQueryColum(columnList)){//如果表单是单查询
			sb.append("<span style=\"float:right\">");
			sb.append("<input id=\""+name+"searchbox\" class=\"easyui-searchbox\"  data-options=\"searcher:"+name+ StringUtil.replaceAll("searchbox,prompt:\'{0}\',menu:\'#", "{0}", MutiLangUtil.getMutiLangInstance().getLang("common.please.input.keyword")) +name+"mm\'\"></input>");
