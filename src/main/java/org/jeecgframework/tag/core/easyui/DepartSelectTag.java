...
public class DepartSelectTag extends TagSupport {
	...
	public StringBuffer end() {
		...
		if (StringUtil.isEmpty(selectedNamesInputId)) {
			selectedNamesInputId = "departname"; // 默认id
		}
		if (StringUtil.isEmpty(selectedIdsInputId)) {
			selectedIdsInputId = "orgIds"; // 默认id
		}
		String lblDepartment = MutiLangUtil.getMutiLangInstance().getLang("common.department");
		if(StringUtil.isEmpty(lblDepartment)){
			lblDepartment = "组织机构";
		}
		
		if(StringUtil.isEmpty(departNameInputWidth)){
			departNameInputWidth = "80px";
		}
		
		if(StringUtil.isEmpty(windowWidth)){
			windowWidth = "400px";
		}
		
		if(StringUtil.isEmpty(windowHeight)){
			windowHeight = "350px";
		}
