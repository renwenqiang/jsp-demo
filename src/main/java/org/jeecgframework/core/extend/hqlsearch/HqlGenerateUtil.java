...
//		--author：龙金波 ------start---date：201504022--------for：增加一个特殊sql参数处理---------------------------------- 
	 ...
	 * @param alias
	 */
	private static void installHqlJoinAlias(CriteriaQuery cq, Object searchObj, Map<String, TSDataRule> ruleMap,
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			try {
				...
				if (type.contains("class java.lang") || type.contains("class java.math")) {
					...
					if (value != null && !value.equals("")) {
						...
						if (HqlRuleEnum.LIKE.equals(rule) && (!(value + "").contains("*"))
								&& !"class java.lang.Integer".contains(type)) {
				...
				} else if (!GenericsUtils.isJavaClass(origDescriptors[i].getPropertyType())) {
	...
	private static void addRuleToCriteria(TSDataRule tsDataRule, String aliasName, Class propertyType,
			CriteriaQuery cq) {
	...
	public static String getSql(List<QueryCondition> list, String tab, Class claszz) {
		...
		for (QueryCondition c : list) {
			...
			if (c.getChildren() != null) {
				...
				sb.append(getSql(c.getChildren(), tab + "\t", claszz));
				sb.append(tab + ")\r\n");
			}
		}
		return sb.toString();
