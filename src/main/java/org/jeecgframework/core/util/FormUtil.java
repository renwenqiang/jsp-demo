...
//					"value").toString();
	...
	private static String GetRadios(JSONObject item,
		...
		for (int f = 0; f < radiosOptions.size(); f++) {
			...
			if (value == null) {
				String check = (json.containsKey("checked") && json
	...
	private static String GetMacros(JSONObject item,
		...
		if ("view".equals(action))
			return value.replace("${userId}", ContextHolderUtils.getLoginUser().getId())
			.replace("${userName}",  ContextHolderUtils.getLoginUser().getUserName());
		if(value!=null){
	...
	private static String GetListctrl(JSONObject item,
		...
		JSONObject dataValue = JSON.parseObject(valuetest);
		...
		for (int row = 0; row < rowCount; row++) {
			...
			JSONArray rowValue = (dataValue != null && dataValue
					.containsKey(name + row)) ? dataValue.getJSONArray(name + row)
					: null;
