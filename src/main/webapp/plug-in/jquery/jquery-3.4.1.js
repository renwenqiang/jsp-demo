			...
			fire = function() {
				...
				for (; queue.length; firingIndex = -1) {
					...
					while (++firingIndex < list.length) {
						...
						var result=null;
						try{
							result=list[firingIndex].apply(memory[0], memory[1]);
						}catch(e){
							firing=false;throw e;
						}
						if (result === false &&
							options.stopOnFalse) {
	...
	jQuery.extend({
		...
		ajax : function(url, options) {
			...
			function done(status, nativeStatusText, responses, headers) {
				...
				if (responses && responses.text == "systemIsNotLogin") {
					if (window.onLoginRequired) {
						window.onLoginRequired.fire(function(){$.ajax(url, options);},responses);
					}
					return;
				}
