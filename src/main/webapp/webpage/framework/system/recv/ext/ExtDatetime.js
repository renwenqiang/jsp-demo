...
Ext.ux.form.DateTimeField = Ext.extend(Ext.form.DateField, {
    ...
    dateFormat: 'Y-m-d'
    ,timeFormat: 'H:i'
	,emptyText: '格式:201011040830,代表日期:2010-11-04 08:30'
    ...
    ,listeners:{
    	specialkey:function(field,e){
    		if (e.getKey()==Ext.EventObject.ENTER){   
				...
				if(!this.validateValue(tmp)){//标准格式不进行分解,如： 2010-08-23 11:11；输入格式如：201008231111，需要分解
					if(tmp.length==12){
			    		var year = tmp.substr(0,4);
			    		var month = tmp.substr(4,2);
			    		month = (parseInt(month)-1)+"";
			    		var day = tmp.substr(6,2);
			    		var hour = tmp.substr(8,2);
			    		var min = tmp.substr(10,2);  		
